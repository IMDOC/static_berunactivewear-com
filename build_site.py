...
def parse_article(path: Path) -> dict | None:
    ...
    tags = raw_tags
    read_time = m.get('twitter:data1') or '5 min read'
...
def render_card(art: dict) -> str:
    ...
    date = art.get('date', '')
    read_time = html_escape(art.get('readTime', '5 min read'))
    url = art.get('url', '#')
    ...
    cat_link = f'<a href="/category/{cat_slug}" style="color:#DC2626;text-decoration:none;">{category}</a>' if cat_slug else category
  ...
  <div class="blog-card-body">
    ...
    <a href="{url}" class="blog-card-read-more">Read More <i class="bi bi-arrow-right"></i></a>
  </div>
</article>'''
...
def ssr_blog_list(root: Path, articles: list[dict], categories_manifest: dict, authors_manifest: dict, tags_manifest: dict) -> bool:
    ...
    cards = '\n'.join(render_card(a) for a in articles) if articles else \
            '<p class="empty-state">New articles coming soon.</p>'
...
def ssr_category_pages(root: Path, articles: list[dict], categories_manifest: dict, authors_manifest: dict, tags_manifest: dict, brand: str, domain: str, topbar_config: str) -> int:
    ...
    for cat in categories_manifest.get('categories', []):
        ...
        seo_title = seo.get('title') or f"{name} — Articles | {brand}"
...
def render_sidebar(current_type: str, current_slug: str, categories: list[dict], tags: list[dict], authors: list[dict]) -> str:
    ...
    for a in authors:
        ...
        author_items.append(f'''<li class="{active}"><a href="/author/{a['slug']}">
            <img src="{html_escape(avatar)}" alt="{html_escape(a['name'])}" loading="lazy">
    ...
    return f'''<aside class="blog-sidebar">
      <div class="sidebar-widget">
        <h3 class="sidebar-title"><i class="bi bi-folder2-open"></i> Categories</h3>
        <ul class="sidebar-list sidebar-categories">{cat_html}</ul>
      </div>
      <div class="sidebar-widget">
        <h3 class="sidebar-title"><i class="bi bi-tags"></i> Popular Tags</h3>
        <div class="sidebar-tags">{tag_html}</div>
      </div>
      <div class="sidebar-widget">
        <h3 class="sidebar-title"><i class="bi bi-people"></i> Authors</h3>
        <ul class="sidebar-list sidebar-authors">{author_html}</ul>
      </div>
      <div class="sidebar-widget sidebar-cta">
        <h3>Need a Factory Quote?</h3>
        <p>MOQ from 50 pcs · OEM & ODM · Global shipping.</p>
    ...
    <div class="container">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-9 order-lg-2 order-1">
                <h2 class="articles-heading">Articles in {name}</h2>
                <div class="row g-4">
                    <!-- CATEGORY_GRID_START -->
                    <!-- CATEGORY_GRID_END -->
                </div>
...
<script>
    window.FooterConfig = {{
        ...
        backgroundColor: '#1F2937', textColor: '#ffffff', linkColor: '#bbbbbb', brandColor: '#DC2626',
...
<section class="page-hero" style="background: linear-gradient(135deg, #1F2937 0%, #374151 100%);">
    <div class="container page-hero-inner">
...
<section class="page-section-wrap">
    <div class="container">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-9 order-lg-2 order-1">
                <h2 class="articles-heading">Articles tagged {tag_display}</h2>
                <div class="row g-4">
                    <!-- TAG_GRID_START -->
                    <!-- TAG_GRID_END -->
                </div>
...
def build_tag_pages(root: Path, articles: list[dict], tags_manifest: dict, categories_manifest: dict, authors_manifest: dict, brand: str, domain: str, topbar_config: str) -> int:
    ...
    for tag_obj in tags_manifest.get('tags', []):
        slug = tag_obj['slug']
        name = tag_obj['name']
        desc = tag_obj.get('description', f'Articles tagged {name}.')
        arts = by_slug.get(slug, [])
        ...
        cards = '\n'.join(render_card(a) for a in arts) if arts else \
                f'<p class="empty-state">No articles tagged <strong>{html_escape(name)}</strong> yet.</p>'
...
def ssr_author_pages(root: Path, articles: list[dict], authors_manifest: dict) -> int:
    ...
    for author in authors_manifest.get('authors', []):
        ...
        cards = '\n'.join(render_card(a) for a in arts) if arts else \
                f'<p class="empty-state">New articles by {author["name"]} will appear here.</p>'
        html = target.read_text(encoding='utf-8')
...
SITEMAP_SKIP = {'404.html', '500.html', 'privacy.html', 'terms.html'}
SITEMAP_PRIORITIES = {
    '': 1.0,              # /
    'index': 1.0,
    'blog': 0.9,
